@extends('layouts.app')

@section('title', (t('Grains')) . ' | GrowSmart')

@push('styles')

<style>
    .crop-page {
        padding-bottom: 25px;
    }

    .crop-hero {
        position: relative;
        padding: 19px 24px;
        margin-bottom: 21px;
        border-radius: 17px;
        background: linear-gradient(135deg, #edf7f1 0%, #ffffff 65%, #f5faf7 100%);
        border: 1px solid #dce9e1;
        overflow: hidden;
    }

    .crop-hero::after {
        content: "";
        position: absolute;
        width: 160px;
        height: 160px;
        right: -52px;
        top: -65px;
        border-radius: 50%;
        background: rgba(86, 135, 104, 0.08);
    }

    .crop-page-title {
        position: relative;
        z-index: 1;
        color: var(--dark-green);
        font-size: 27px;
        font-weight: 800;
        margin: 0 0 6px;
        letter-spacing: -0.3px;
    }

    .crop-page-subtitle {
        position: relative;
        z-index: 1;
        color: var(--gray);
        font-size: 13px;
        margin: 0;
        max-width: 620px;
        line-height: 1.6;
    }

    .crop-section {
        border: 1px solid var(--border);
        border-radius: 18px;
        background: var(--white);
        padding: 18px;
        box-shadow: 0 7px 25px rgba(23, 59, 50, 0.07);
    }

    .crop-section-header {
        display: flex;
        align-items: center;
        justify-content: space-between;
        gap: 15px;
        margin-bottom: 15px;
    }

    .crop-section-title {
        margin: 0;
        color: var(--dark-green);
        font-size: 17px;
        font-weight: 700;
    }

    .crop-count {
        display: inline-flex;
        align-items: center;
        justify-content: center;
        min-width: 36px;
        height: 28px;
        padding: 0 9px;
        border-radius: 20px;
        background: #edf6f0;
        color: var(--dark-green);
        font-size: 12px;
        font-weight: 700;
    }

    .crop-card-link {
        display: block;
        height: 100%;
        text-decoration: none;
        color: inherit;
    }

    .crop-card {
        width: 100%;
        height: 290px;
        border-radius: 15px;
        overflow: hidden;
        position: relative;
        cursor: pointer;
        border: 1px solid #e4ebe7;
        background: #ffffff;
        display: flex;
        flex-direction: column;
        transition:
            transform 0.3s ease,
            box-shadow 0.3s ease,
            border-color 0.3s ease;
    }

    .crop-card:hover {
        transform: translateY(-6px);
        border-color: #b8d0c1;
        box-shadow: 0 15px 32px rgba(23, 59, 50, 0.15);
    }

    .crop-image-wrapper {
        position: relative;
        width: 100%;
        height: 195px;
        overflow: hidden;
        background: #f2f6f3;
    }

    .crop-card img {
        width: 100%;
        height: 100%;
        object-fit: cover;
        object-position: center;
        display: block;
        transition: transform 0.5s ease;
    }

    .crop-card:hover img {
        transform: scale(1.07);
    }

    .crop-image-overlay {
        position: absolute;
        inset: 0;
        background: linear-gradient(
            to top,
            rgba(0, 0, 0, 0.28),
            transparent 45%
        );
        pointer-events: none;
    }

    .view-badge {
        position: absolute;
        right: 10px;
        bottom: 10px;
        display: flex;
        align-items: center;
        gap: 5px;
        padding: 6px 10px;
        border-radius: 20px;
        background: rgba(255, 255, 255, 0.94);
        color: var(--dark-green);
        font-size: 10px;
        font-weight: 700;
        opacity: 0;
        transform: translateY(7px);
        transition: all 0.3s ease;
    }

    .crop-card:hover .view-badge {
        opacity: 1;
        transform: translateY(0);
    }

    .card-info {
        flex: 1;
        padding: 12px 13px;
        display: flex;
        flex-direction: column;
        justify-content: center;
        align-items: center;
        text-align: center;
        background: #ffffff;
    }

    .crop-title {
        font-weight: 700;
        font-size: 15px;
        color: var(--dark-green);
        line-height: 1.3;
    }

    .crop-type {
        color: var(--gray);
        font-size: 10px;
        margin-top: 6px;
        padding: 4px 9px;
        border-radius: 15px;
        background: #f1f5f2;
    }

    .empty-state {
        padding: 45px 20px;
        text-align: center;
        border: 1px dashed #cbd9d0;
        border-radius: 15px;
        background: #fafcfb;
    }

    .empty-state-icon {
        width: 58px;
        height: 58px;
        margin: 0 auto 13px;
        display: flex;
        align-items: center;
        justify-content: center;
        border-radius: 50%;
        background: #edf6f0;
        color: var(--dark-green);
        font-size: 24px;
    }

    .empty-state h5 {
        color: var(--dark-green);
        font-weight: 700;
        margin-bottom: 6px;
    }

    .empty-state p {
        color: var(--gray);
        font-size: 12px;
        margin: 0;
    }

    @media (max-width: 992px) {

        .crop-hero {
            padding: 18px 21px;
        }

        .crop-page-title {
            font-size: 25px;
        }

        .crop-section {
            padding: 16px;
        }

        .crop-card {
            height: 275px;
        }

        .crop-image-wrapper {
            height: 185px;
        }
    }

    @media (max-width: 768px) {

        .crop-hero {
            padding: 17px 19px;
            border-radius: 15px;
        }

        .crop-page-title {
            font-size: 22px;
        }

        .crop-page-subtitle {
            font-size: 12px;
        }

        .crop-section {
            padding: 12px;
            border-radius: 15px;
        }

        .crop-section-title {
            font-size: 15px;
        }

        .crop-card {
            height: 245px;
            border-radius: 13px;
        }

        .crop-image-wrapper {
            height: 165px;
        }

        .crop-title {
            font-size: 13px;
        }

        .view-badge {
            display: none;
        }
    }

    @media (max-width: 576px) {

        .crop-hero {
            padding: 14px 16px;
            margin-bottom: 17px;
        }

        .crop-page-title {
            font-size: 20px;
        }

        .crop-page-subtitle {
            font-size: 11px;
            line-height: 1.5;
        }

        .crop-section {
            padding: 8px;
        }

        .crop-section-header {
            margin-bottom: 11px;
        }

        .crop-section-title {
            font-size: 14px;
        }

        .crop-count {
            min-width: 30px;
            height: 25px;
            font-size: 10px;
        }

        .crop-card {
            height: 215px;
            border-radius: 11px;
        }

        .crop-image-wrapper {
            height: 140px;
        }

        .card-info {
            padding: 8px;
        }

        .crop-title {
            font-size: 11px;
        }

        .crop-type {
            font-size: 9px;
            margin-top: 4px;
            padding: 3px 7px;
        }
    }
</style>

@endpush

@section('content')

<div class="crop-page">

<div class="crop-hero">

    <h1 class="crop-page-title">
        {{ t('Grains') }}
    </h1>

    <p class="crop-page-subtitle">
        {{ t('Explore different types of grains and discover useful agricultural information, growing guidance, and crop details.') }}
    </p>

</div>

<div class="crop-section">

    <div class="crop-section-header">

        <h2 class="crop-section-title">
            {{ t('Available Grains') }}
        </h2>

        <span class="crop-count">
            {{ $crops->count() }}
        </span>

    </div>

    <div class="row g-3">

        @forelse($crops as $crop)

            <div class="col-xl-3 col-lg-4 col-md-4 col-sm-6 col-6">

                <a
                    href="{{ route('crop.show', $crop->id) }}"
                    class="crop-card-link"
                >

                    <div class="crop-card">

                        <div class="crop-image-wrapper">

                            <img
                                src="{{ asset('images/' . $crop->image) }}"
                                alt="{{ local_text($crop, 'name') }}"
                                loading="lazy"
                                onerror="this.src='{{ asset('images/default-crop.jpg') }}'"
                            >

                            <div class="crop-image-overlay"></div>

                            <div class="view-badge">

                                <i class="bi bi-arrow-right"></i>

                                {{ t('View Details') }}

                            </div>

                        </div>

                        <div class="card-info">

                            <div class="crop-title">
                                {{ local_text($crop, 'name') }}
                            </div>

                            @if($crop->type)

                                <div class="crop-type">
                                    {{ local_text($crop, 'type') }}
                                </div>

                            @endif

                        </div>

                    </div>

                </a>

            </div>

        @empty

            <div class="col-12">

                <div class="empty-state">

                    <div class="empty-state-icon">
                        <i class="bi bi-flower1"></i>
                    </div>

                    <h5>
                        {{ t('No Grains Available') }}
                    </h5>

                    <p>
                        {{ t('There are currently no grains available to display.') }}
                    </p>

                </div>

            </div>

        @endforelse

    </div>

</div>

</div>

@endsection
