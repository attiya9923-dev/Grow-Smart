...
<div class="expert-users-container">
    ...
    <h2>🍎 {{ t('Users Fruit Questions') }}</h2>
    <p class="intro">
        {{ t('Each user stays in this list until you manually remove them. Answered questions are still visible.') }}
    </p>
        ...
        <div class="user-card">
            <div>
                ...
                <div class="question-count">
                    {{ $user->question_count }}
                    {{ $user->question_count == 1 ? t('Question') : t('Questions') }}
                </div>

                @if($user->question_count > 0 && $user->answered_question_count == $user->question_count)
                    <span class="completed">✓ {{ t('All questions answered') }}</span>
                @endif
            </div>

            <div class="user-actions">
                <a href="{{ route('expert.fruit.user.questions', $user->id) }}" class="view-btn">
                    {{ t('View Questions') }}
                </a>

                <div class="more-wrap">
                    <button type="button" class="more-btn" aria-label="{{ t('More options') }}">⋮</button>
                    <div class="more-menu">
                        <form action="{{ route('expert.user.hide', $user->id) }}" method="POST" class="hide-user-form" data-category="fruit" data-user-name="{{ $user->name }}">
                            @csrf
                            @method('DELETE')
                            <input type="hidden" name="category" value="fruit">
                            <button type="submit" class="delete-user">🗑 {{ t('Delete from list') }}</button>
                        </form>
                    </div>
                </div>
            </div>
    ...
    @empty
        <div class="no-users">
            {{ t('No users have questions in this category.') }}
        </div>
    @endforelse
</div>


<div class="expert-delete-modal" id="expertDeleteModal-fruit" aria-hidden="true">
    ...
    <div class="expert-delete-box" role="dialog" aria-modal="true">
        ...
        <h3>{{ t('Remove user from list?') }}</h3>
        <p id="expertDeleteMessage-fruit">{{ t('The user account and questions will not be deleted.') }}</p>
        <div class="expert-delete-actions">
            <button type="button" class="expert-cancel-delete">{{ t('No') }}</button>
            <button type="button" class="expert-confirm-delete">{{ t('Yes, Delete') }}</button>
        </div>
    </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const deleteModal = document.getElementById('expertDeleteModal-fruit');
